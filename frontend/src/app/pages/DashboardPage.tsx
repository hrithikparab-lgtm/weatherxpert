...
export function DashboardPage({ selectedUtility }: DashboardPageProps) {
  ...
  return (
    <div className="flex-1 bg-background">
      ...
      {/* ── Original Dashboard content (legacy, hidden) ── */}
      ...
      <div className="h-full">
          ...
          <div className="px-6 py-5">
            <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Executive overview of weather intelligence
                </p>
              </div>
            </div>

            {/* Filter Row - Styled like reference image */}
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <FilterDropdown
                label="Parameter"
                value={parameter}
                options={["Wind Speed", "Wind Direction", "Wind Gust", "Ambient Temperature", "Ambient Pressure", "Relative Humidity", "Air Density", "Cloud Cover", "Rainfall", "Precipitation", "Dew Point Temperature"]}
        ...
        <div className="p-6 space-y-8">
          {/* TAB NAVIGATION FOR KPI SECTIONS */}
          <div className="flex items-center gap-1 p-1 bg-white/40 dark:bg-black/20 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-[16px] w-fit shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)]">
            <button
              onClick={() => setActiveKPITab("overview")}
              className={`px-4 py-2 text-[13px] font-medium rounded-[12px] transition-all duration-200 ${
                activeKPITab === "overview" ? "bg-white dark:bg-white/15 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-white/50 dark:hover:bg-white/5"
              }`}
            >
              KPI Overview
            </button>
            <button
              onClick={() => setActiveKPITab("parameters")}
              className={`px-4 py-2 text-[13px] font-medium rounded-[12px] transition-all duration-200 ${
                activeKPITab === "parameters" ? "bg-white dark:bg-white/15 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-white/50 dark:hover:bg-white/5"
              }`}
            >
              Weather Parameters
            </button>
            <button
              onClick={() => setActiveKPITab("accuracy")}
              className={`px-4 py-2 text-[13px] font-medium rounded-[12px] transition-all duration-200 ${
                activeKPITab === "accuracy" ? "bg-white dark:bg-white/15 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-white/50 dark:hover:bg-white/5"
              }`}
            >
              Error Metrics
            </button>
          ...
          <div className="relative min-h-[280px]">
            <AnimatePresence mode="wait">
              {activeKPITab === "overview" && (
                ...
                >
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      <h2 className="text-[20px] font-semibold text-foreground tracking-tight">Key Performance Indicators</h2>
                      <p className="text-[13px] text-muted-foreground/80 mt-1">Real-time metrics overview</p>
                    </div>
              ...
              <HeroKPICard
                ...
                isUp={kpiData.forecastAccuracy.isUp}
                sparklineData={generateSparklineData()}
                tooltip="Overall accuracy of weather forecasts compared to actual observations"
                icon={<Target className="w-6 h-6" />}
                accentColor="#6366f1"
              />
              <HeroKPICard
                title="Solar Potential"
                value={`${kpiData.solarPotential.value} ${kpiData.solarPotential.unit}`}
                trend={kpiData.solarPotential.trend}
                isUp={kpiData.solarPotential.isUp}
                sparklineData={generateSparklineData(7, 2)}
                tooltip="Total solar irradiance potential for energy generation"
                icon={<Sun className="w-6 h-6" />}
                accentColor="#f59e0b"
              />
              <HeroKPICard
                title="Extreme Weather Events"
                value={`${kpiData.extremeEvents.value} Active`}
                trend={kpiData.extremeEvents.trend}
                isUp={kpiData.extremeEvents.isUp}
                sparklineData={generateSparklineData(7, 4)}
                tooltip="Currently active extreme weather events requiring attention"
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* SECTION 2: ALL PARAMETER KPIs — Premium Redesign */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          {activeKPITab === "parameters" && (
          <motion.div
            key="tab-parameters"
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.3 }}
            className="p-6 rounded-[28px] bg-white/40 dark:bg-black/10 backdrop-blur-2xl border border-white/60 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] relative overflow-hidden"
          >
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-[20px] font-semibold text-foreground tracking-tight">Weather Parameters Overview</h2>
                  <p className="text-[13px] text-muted-foreground/80 mt-1">Real-time measurements and forecasts across all parameters</p>
                </div>
                <button className="px-4 py-2 text-[12px] bg-white/50 dark:bg-white/5 hover:bg-white/80 dark:hover:bg-white/10 text-foreground rounded-full transition-colors flex items-center gap-1.5 border border-black/5 dark:border-white/10 shadow-sm backdrop-blur-md">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <CompactParameterCard
                  title="Temperature"
                  value={allParameterKPIs.temperature.value}
                  unit={allParameterKPIs.temperature.unit}
                  trend={allParameterKPIs.temperature.trend}
                  isUp={allParameterKPIs.temperature.isUp}
                  forecast={allParameterKPIs.temperature.forecast}
                  accuracy={allParameterKPIs.temperature.accuracy}
                ...
                <CompactParameterCard
                  title="Wind Speed"
                  value={allParameterKPIs.windSpeed.value}
                  unit={allParameterKPIs.windSpeed.unit}
                  trend={allParameterKPIs.windSpeed.trend}
                  isUp={allParameterKPIs.windSpeed.isUp}
                  forecast={allParameterKPIs.windSpeed.forecast}
                  accuracy={allParameterKPIs.windSpeed.accuracy}
                ...
                <CompactParameterCard
                  title="Humidity"
                  value={allParameterKPIs.humidity.value}
                  unit={allParameterKPIs.humidity.unit}
                  trend={allParameterKPIs.humidity.trend}
                  isUp={allParameterKPIs.humidity.isUp}
                  forecast={allParameterKPIs.humidity.forecast}
                  accuracy={allParameterKPIs.humidity.accuracy}
                ...
                <CompactParameterCard
                  title="Precipitation"
                  value={allParameterKPIs.precipitation.value}
                  unit={allParameterKPIs.precipitation.unit}
                  trend={allParameterKPIs.precipitation.trend}
                  isUp={allParameterKPIs.precipitation.isUp}
                  forecast={allParameterKPIs.precipitation.forecast}
                  accuracy={allParameterKPIs.precipitation.accuracy}
                  icon={<CloudRain className="w-4 h-4" />}
                  color="#6366f1"
                />

                {/* Irradiance (Conditional) */}
                {showSolarFeatures && (
                  <CompactParameterCard
                    title="Irradiance"
                    value={allParameterKPIs.irradiance.value}
                    unit={allParameterKPIs.irradiance.unit}
                    trend={allParameterKPIs.irradiance.trend}
                    isUp={allParameterKPIs.irradiance.isUp}
                    forecast={allParameterKPIs.irradiance.forecast}
                    accuracy={allParameterKPIs.irradiance.accuracy}
                ...
                <CompactParameterCard
                  title="Cloud Cover"
                  value={allParameterKPIs.cloudCover.value}
                  unit={allParameterKPIs.cloudCover.unit}
                  trend={allParameterKPIs.cloudCover.trend}
                  isUp={allParameterKPIs.cloudCover.isUp}
                  forecast={allParameterKPIs.cloudCover.forecast}
                  accuracy={allParameterKPIs.cloudCover.accuracy}
                ...
                <CompactParameterCard
                  title="UV Index"
                  value={allParameterKPIs.uvIndex.value}
                  unit={allParameterKPIs.uvIndex.unit}
                  trend={allParameterKPIs.uvIndex.trend}
                  isUp={allParameterKPIs.uvIndex.isUp}
                  forecast={allParameterKPIs.uvIndex.forecast}
                  accuracy={allParameterKPIs.uvIndex.accuracy}
                ...
                <CompactParameterCard
                  title="Air Quality"
                  value={allParameterKPIs.airQuality.value}
                  unit={allParameterKPIs.airQuality.unit}
                  trend={allParameterKPIs.airQuality.trend}
                  isUp={allParameterKPIs.airQuality.isUp}
                  forecast={allParameterKPIs.airQuality.forecast}
                  accuracy={allParameterKPIs.airQuality.accuracy}
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* SECTION 3: FORECAST ACCURACY METRICS — Premium Redesign */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          {activeKPITab === "accuracy" && (
          <motion.div
            key="tab-accuracy"
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="mb-5">
              <h2 className="text-[20px] font-semibold text-foreground tracking-tight">Forecast Error Metrics</h2>
              <p className="text-[13px] text-muted-foreground/80 mt-1">Statistical accuracy indicators for {parameter} predictions</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <MetricCard
                ...
                value={`${accuracyMetrics.mae.value} ${accuracyMetrics.mae.unit}`}
                trend={accuracyMetrics.mae.trend}
                isUp={accuracyMetrics.mae.isUp}
                sparklineData={generateSparklineData(10)}
                tooltip="Mean Absolute Error between forecast and observed weather values"
                color="#6366f1"
                maxValue={10}
              />
              <MetricCard
                ...
                value={`${accuracyMetrics.rmse.value} ${accuracyMetrics.rmse.unit}`}
                trend={accuracyMetrics.rmse.trend}
                isUp={accuracyMetrics.rmse.isUp}
                sparklineData={generateSparklineData(10, 3)}
                tooltip="Root Mean Square Error measuring forecast deviation"
                color="#f59e0b"
                maxValue={15}
              />
              <MetricCard
                ...
                value={`${accuracyMetrics.mape.value} ${accuracyMetrics.mape.unit}`}
                trend={accuracyMetrics.mape.trend}
                isUp={accuracyMetrics.mape.isUp}
                sparklineData={generateSparklineData(10, 6)}
                tooltip="Percentage-based error metric for forecast accuracy"
                color="#10b981"
                maxValue={25}
              />
            </div>
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* SECTION 4: FORECAST HORIZON ACCURACY */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          <ForecastHorizonAccuracySection />

          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* SECTION 5: SOLAR INTELLIGENCE OVERVIEW (Conditional) */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          {showSolarFeatures && (
            <motion.div
            ...
            >
              <div className="mb-5">
                <h2 className="text-lg font-bold text-foreground">Solar Intelligence</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Real-time solar energy metrics and forecasts</p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex items-center justify-center h-52">
                      <GaugeChart value={0.85} label="CSI" />
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="mb-4">
                      <h3 className="text-base font-semibold text-foreground">Solar Potential Today</h3>
                      <p className="text-xs text-muted-foreground mt-1">
                        Total irradiance energy potential
                      </p>
                    </div>
                    <div className="flex flex-col items-center justify-center h-52">
                      <div className="relative mb-4">
                        <div className="absolute inset-0 rounded-full bg-amber-400/20 animate-ping" style={{ animationDuration: '3s' }} />
                        <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-amber-300 to-orange-500 flex items-center justify-center shadow-[0_0_40px_rgba(251,191,36,0.5)]">
                          <Sun className="w-10 h-10 text-white" />
                        </div>
                      </div>
                      <div className="text-5xl font-bold bg-gradient-to-r from-amber-500 to-orange-500 bg-clip-text text-transparent">7.5</div>
                      <div className="text-sm text-muted-foreground mt-1.5">kWh/m²</div>
                      <div className="flex items-center gap-1.5 mt-4 px-3 py-1.5 rounded-full bg-green-500/10 text-green-600 dark:text-green-400 text-sm font-medium border border-green-500/20">
                ...
                >
                  ...
                  <div className="relative z-10">
                    <div className="mb-4">
                      <h3 className="text-base font-semibold text-foreground">GHI vs DHI</h3>
                      <p className="text-xs text-muted-foreground mt-1">
                        Global vs Diffuse Horizontal Irradiance
                      </p>
                    </div>
                    {/* Gradients defined outside Recharts to avoid duplicate-key warnings */}
                    <svg width="0" height="0" className="absolute overflow-hidden" aria-hidden="true">
                      <defs>
                        <linearGradient id="dash-ghiGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop key="ghi-stop-1" offset="5%" stopColor="#f59e0b" stopOpacity={0.5} />
                          <stop key="ghi-stop-2" offset="95%" stopColor="#f59e0b" stopOpacity={0} />
                        </linearGradient>
                        <linearGradient id="dash-dhiGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop key="dhi-stop-1" offset="5%" stopColor="#6366f1" stopOpacity={0.5} />
                          <stop key="dhi-stop-2" offset="95%" stopColor="#6366f1" stopOpacity={0} />
                        </linearGradient>
                      </defs>
                    </svg>
                    <div className="h-52">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={generateGHIData(selectedLocation, timeRange)}>
                          ...
                          <Area type="monotone" dataKey="GHI" stroke="#f59e0b" strokeWidth={2.5} fill="url(#dash-ghiGrad)" name="GHI (W/m²)" dot={false} />
                          <Area type="monotone" dataKey="DHI" stroke="#6366f1" strokeWidth={2.5} fill="url(#dash-dhiGrad)" name="DHI (W/m²)" dot={false} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* SECTION 6 & 7: EXTREME WEATHER & FORECAST CONFIDENCE */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex-1">
                      <h3 className="text-base font-semibold text-foreground">Extreme Weather Detection</h3>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative">
                        <button
                          onClick={() => setWeatherParameterDropdownOpen(!weatherParameterDropdownOpen)}
                          className="px-3 py-1.5 text-xs bg-background/60 backdrop-blur-sm border border-border rounded-lg hover:bg-accent transition-colors flex items-center gap-2 shadow-sm"
                        >
                          <span className="text-muted-foreground font-medium">{weatherParameter}</span>
                          <ChevronDown className="w-3.5 h-3.5 text-muted-foreground" />
                        </button>
                        {weatherParameterDropdownOpen && (
                          <div className="absolute right-0 mt-2 w-56 bg-popover border border-border rounded-lg shadow-lg z-50 py-1 backdrop-blur-xl">
                            {(["All Parameters", "Wind Speed", "Wind Direction", "Wind Gust", "Ambient Temperature", "Ambient Pressure", "Relative Humidity", "Air Density", "Cloud Cover", "Rainfall", "Precipitation", "Dew Point Temperature"] as WeatherParameter[]).map((param) => (
                              <button
                                key={param}
                                onClick={() => {
                                  setWeatherParameter(param);
                                  setWeatherParameterDropdownOpen(false);
                                }}
                                className={`w-full px-3 py-2 text-left text-xs hover:bg-accent transition-colors ${
                                  weatherParameter === param ? "bg-accent text-accent-foreground font-semibold" : "text-foreground"
                                }`}
                              >
                                {param}
                              </button>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-xs bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors flex items-center gap-1.5 shadow-sm font-medium"
                      >
                        <Eye className="w-3.5 h-3.5" />
                        View Alerts
                      </button>
                  ...
                  {/* Vertical scrollable card container - shows 3 cards at a time */}
                  <div className="overflow-y-auto scrollbar-hide max-h-[310px] space-y-3 pr-2">
                    {generateExtremeWeatherEvents(selectedLocation, timeRange, selectedProvider)
                      .filter(event => weatherParameter === "All Parameters" || event.parameter === weatherParameter)
                      .slice(0, 5)
                      .map((event) => (
                      <motion.div
                        ...
                        className={`flex items-center justify-between p-4 rounded-xl border backdrop-blur-sm transition-all hover:shadow-md group cursor-default ${
                          event.severity === "High"
                            ? "border-red-200/50 dark:border-red-800/30 bg-red-50/40 dark:bg-red-900/10 hover:bg-red-50/60 dark:hover:bg-red-900/20"
                            : event.severity === "Medium"
                            ? "border-amber-200/50 dark:border-amber-800/30 bg-amber-50/40 dark:bg-amber-900/10 hover:bg-amber-50/60 dark:hover:bg-amber-900/20"
                            : "border-green-200/50 dark:border-green-800/30 bg-green-50/40 dark:bg-green-900/10 hover:bg-green-50/60 dark:hover:bg-green-900/20"
                        }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-3 h-3 rounded-full flex-shrink-0 ${
                            event.severity === "High" ? "bg-red-500 shadow-[0_0_12px_rgba(239,68,68,0.7)]" :
                            event.severity === "Medium" ? "bg-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.7)]" :
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground flex items-center gap-3 mt-1">
                              ...
                              <span className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded text-[10px] font-medium border border-blue-200 dark:border-blue-800">
                                {event.parameter}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`px-2.5 py-1 text-xs rounded-full font-semibold ${
                            event.severity === "High" ? "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400 border border-red-200 dark:border-red-800" :
                            event.severity === "Medium" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400 border border-amber-200 dark:border-amber-800" :
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  {/* Header */}
                  <div className="mb-3">
                    <h3 className="text-base font-semibold text-foreground">Forecast Confidence</h3>
                    <p className="text-xs text-muted-foreground mt-0.5">Overall reliability score</p>
                  </div>

                  {/* Redesigned Gauge — integrates today + 7-day */}
                  <div className="flex-1 w-full">
                    <FCGauge 
                      today={confidenceScores.today} 
                      sevenDay={confidenceScores.sevenDay} 
                    />
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-border/50">
                    <div className="space-y-1.5 text-xs">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium text-foreground truncate ml-2 max-w-[140px]" title={selectedLocation}>{selectedLocation}</span>
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium text-foreground">{forecastHorizon}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* SECTION 8: WEATHER EVENT TIMELINE */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div className="relative z-10">
              <div className="mb-6">
                <h3 className="text-base font-semibold text-foreground">Weather Event Timeline</h3>
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/30 text-xs">
                    <MapPin className="w-3 h-3 text-blue-600 dark:text-blue-400" />
                    <span className="text-blue-700 dark:text-blue-300 font-medium max-w-[140px] truncate">{selectedLocation}</span>
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/30 text-xs">
                    <Activity className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                    <span className="text-emerald-700 dark:text-emerald-300 font-medium">{selectedProvider}</span>
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 text-xs">
                    <Clock className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                    <span className="text-amber-700 dark:text-amber-300 font-medium">{timeRange}</span>
                  </span>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800/30 text-xs">
                    <Gauge className="w-3 h-3 text-indigo-600 dark:text-indigo-400" />
                    <span className="text-indigo-700 dark:text-indigo-300 font-medium">{forecastHorizon}</span>
                  </span>
                </div>
              </div>
              <div className="relative flex items-start justify-between gap-0 overflow-x-auto pb-2">
                {/* Connecting line */}
                <div className="absolute top-6 left-6 right-6 h-0.5 bg-gradient-to-r from-transparent via-border to-transparent" />
                {generateWeatherTimeline(selectedLocation, timeRange, parameter).map((item, idx) => {
                  const iconColor =
                    item.icon === "sun" ? "text-amber-500" :
                    item.icon === "cloud-rain" ? "text-blue-500" :
                    item.icon === "wind" ? "text-cyan-500" :
                    "text-red-500";
                  const bgColor =
                    item.icon === "sun" ? "from-amber-400/20 to-orange-300/10 border-amber-200/40" :
                    item.icon === "cloud-rain" ? "from-blue-400/20 to-blue-300/10 border-blue-200/40" :
                    item.icon === "wind" ? "from-cyan-400/20 to-cyan-300/10 border-cyan-200/40" :
                    "from-red-400/20 to-red-300/10 border-red-200/40";
                  return (
                    <motion.div
                      key={`timeline-${idx}-${item.event}`}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: idx * 0.1 }}
                      className="relative flex flex-col items-center min-w-[120px] z-10"
                    >
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${bgColor} border backdrop-blur-sm flex items-center justify-center mb-3 shadow-md hover:scale-110 transition-transform`}>
                        {item.icon === "sun" && <Sun className={`w-5 h-5 ${iconColor}`} />}
                        {item.icon === "cloud-rain" && <CloudRain className={`w-5 h-5 ${iconColor}`} />}
                        {item.icon === "wind" && <Wind className={`w-5 h-5 ${iconColor}`} />}
                        {item.icon === "alert" && <AlertCircle className={`w-5 h-5 ${iconColor}`} />}
                      </div>
                      <div className="text-xs font-semibold text-foreground text-center">{item.event}</div>
